...
도시 목록은 두 벌이다. 과제 화면은 `src/data/cityCoords.js`의 여섯 곳을 보고, 메인홈과 최종본은 `src/data/heroCities.js`의 20곳을 본다. 나눠 둔 이유는 메인홈에서 도시를 늘려도 이미 제출한 과제 화면의 내용이 바뀌면 안 되기 때문이다. 누적본은 새 내용을 반영하는 자리라 스무 곳 쪽에 붙였다. 스무 곳 쪽에는 좌표 말고 사진 주소와 사진에 찍힌 명소 이름도 함께 들어 있다.
...
강의는 실습을 열 단원으로 나눠 두고 다시 기본 문법과 확장 문법 두 갈래로 묶어 본다. 1에서 4까지와 9가 기본 문법 쪽이고, Router와 Pinia, Axios, UI 라이브러리처럼 밖에서 가져다 붙이는 5에서 8까지가 확장 문법 쪽이다.
...
앞의 네 단원이 쓰라고 정해 준 라이브러리는 Router와 Pinia, Axios, Element Plus다. 여기서는 교재가 짚지 않은 것을 직접 골라야 해서 Three.js로 메인홈을 다시 만들었다. `/`에 들어가면 도시 사진 카드 스무 장이 3D 구 위에 얹혀 돌아간다.

장식으로 옆에 세운 화면이 아니라 과제에 물려 있다. 카드에 얹히는 기온은 최종본이 쓰는 스토어에서 그대로 읽는다. 카드를 누르면 그 도시의 상세 경로로 넘어간다. 데이터를 처음 불러오는 것도 이 화면이라 스피어를 먼저 보고 목록으로 넘어가면 이미 받아 둔 값이 그대로 얹힌다. Three.js는 강의에서 다루지 않은 라이브러리라 AI 도움을 받았다.

과제 쪽 데이터도 함께 늘려서 도시가 여섯 곳에서 스무 곳이 되었고 도시마다 위키미디어에서 가져온 명소 사진과 그 장소 이름이 붙는다. 예보는 OpenWeather 대신 Open-Meteo에서 받는다. 키 없이 열려 있어서 사흘치 최고 최저 기온을 붙이는 데는 이쪽이 편했다.

Element Plus도 과제 밖으로 더 끌고 나왔다. 트러블슈팅 기록은 `el-collapse`로 날짜별로 접었다. 스피어 카드에는 `el-tooltip`으로 도시와 명소 이름을 달았고 대시보드와 소개 화면을 오가는 줄은 `el-menu`로 만들었다. 과제 7이 쓰지 않은 부품들이다.
